...
public interface FlexFieldProcessor<T> {
   ...
   * @param flexField
   */
  void onFieldClick(FlexField<T> flexField);
   ...
   */
  boolean onChange(FlexField<T> flexField, T newValue, boolean isSelf);

  /**
   * 处理onActivityResult的返回结果，需要在这里主动回调onChange更新
   */
  void onActivityResult(FlexField<T> flexField, Bundle resultData);
}
